...
def process_video(video_path: str, pose_estimator: PoseEstimator, frames_diff: int):
    ...
    while cap.isOpened():
        ...
        if frame_counter % frames_diff == 0:
            try:
                pose_landmarks = pose_estimator(img = frame, revert_channels = True)
...
def video_to_csv(data_path: str,
    ...
    class_names = [folder for folder in os.listdir(data_path) if os.path.isdir(data_path / folder) ]
    print(f'In {str(data_path)} found the folloding classes: {", ".join(class_names)}')
    for class_ in class_names:
        ...
        for video_name in tqdm(list_videos):
            ...
            landmarks_from_video = process_video(video_path=str(path_to_video), pose_estimator=pose_estimator, frames_diff=frames_diff)
            for landmarks in landmarks_from_video:
                for landmark_name, index in wish_landmarks.items():
                    csv_dict[f'{landmark_name}_X'].append(round(landmarks.pose_landmarks.landmark[index].x, 4))
                    csv_dict[f'{landmark_name}_Y'].append(round(landmarks.pose_landmarks.landmark[index].y, 4))
