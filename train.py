...
parser.add_argument('-mn', '--model_name', type=str, default='model' + str(round(time())))
...
train_dataloader = create_dataloader(args.train_path, args.num_records, args.step, args.batch_size, train_shuffle, num_workers)

if args.val_path:
    val_dataloader = create_dataloader(args.val_path, args.num_records, args.step, args.batch_size, val_shuffle, num_workers)
...
if args.balanced_class_weights:
    class_weights = compute_class_weight('balanced', classes = np.unique(train_dataloader.dataset.classes), y = train_dataloader.dataset.classes)
    class_weights = torch.from_numpy(class_weights).type(torch.float32).to(args.device)
...
for epoch in tqdm(range(1, args.num_epochs+1)):
    train_loss, train_acc = train_step(model, train_dataloader, loss_fn, optimizer, args.device)
    
    if epoch % 5 == 0:
        print(f'Epoch: {epoch} | Train loss: {train_loss} | Train acc: {train_acc}')
    
    if args.val_path:
        val_loss, val_acc = val_step(model, val_dataloader, loss_fn, args.device)
        if epoch % 5 == 0:
